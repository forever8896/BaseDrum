...
import type { TrackData, SongData } from './songSchema-new';
export type { TrackData, SongData } from './songSchema-new';
export { 
  TrackDataSchema, 
  SongDataSchema,
  validateSongData,
...
export function toggleStep(track: TrackData, step: number, note?: string): void {
  ...
  if (index > -1) {
    ...
    track.pattern = sortedIndices.map((item: { val: number; idx: number }) => item.val);
    if (track.notes) {
      const newNotes: string[] = [];
      sortedIndices.forEach((item: { val: number; idx: number }) => {
...
export { importSongFromJSON } from './songSchema-new';